...
> 版本: v4.0 | 更新: 2026-02-19
>
> 本文档聚焦用户需求，不涉及技术实现方案。
>
> **重大更新**: v4.0 新增"AI 项目管理"核心场景（未来战略方向）
...
#### 当前（v1.x-v2.x）：个人远程运维控制台

Ergo（二狗）是一个**个人远程运维控制台**，让用户在手机或任何外部网络环境下，能够监控、管理和恢复本地部署的 AI 系统和开发服务。

#### 未来（v2.x+）：AI 项目管理平台 🚀

Ergo 将演进为 **AI 驱动的项目管理平台**，让多个项目由 AI Agent 团队自主协同开发，用户只需 Setup 项目和监控进展，无需人工执行开发任务。
...
- **Setup**: 一键创建项目，自动组建 Agent 团队（产品、开发、QA、设计）
- **监控**: 实时查看所有项目的研发进展、任务状态、代码提交
- **介入**: 只在关键决策点介入（产品方向、架构选型、发布审批）
...
**做（未来 v2.x+）**:
- AI 项目管理：多项目并行管理、Agent 团队协作、进展监控
- 项目 Setup 自动化：一键创建项目和组建 Agent 团队
- 关键决策介入：用户只管方向，Agent 负责执行

**不做**:
- 不替代 OpenClaw Dashboard 的全部管理能力（复杂的 Agent 编排、工作流设计等留给 Dashboard）
- 不替代 Claude Code 本身（在线代码编辑器不是 Ergo 的目标）
- 不做通用项目管理（Ergo 专注 AI Agent 驱动的项目）
- 不替代 GitHub/GitLab（代码托管和版本控制仍用现有工具）
...
### 场景域 C：AI 项目管理（v2.x+ 核心场景）🚀

#### C1. 项目 Setup — "新建乐队"

**频率**: 每周 1-2 次

**故事**: 有一个新的想法或需求，想快速启动一个项目由 AI 团队开发。

**用户期望**:
- 一键创建项目（名称、描述、目标）
- 自动组建 Agent 团队：
  - 产品经理 Agent：负责 PRD 编写、需求澄清
  - 全栈工程师 Agent：负责代码实现
  - QA Agent：负责测试和质量把控
  - UI 设计 Agent（可选）：负责界面设计
- 自动初始化项目仓库（Git）
- 自动生成初始 PRD 框架

**成功标准**: 5 分钟内完成项目 Setup，Agent 团队开始自主工作。
...
**故事**: 同时管理 3-5 个项目，需要快速了解所有项目的整体进展。

**用户期望**:
- **多项目 Dashboard**:
  - 卡片式布局，每个项目一张卡片
  - 显示关键指标：进度条（60%）、今日 commits（5 个）、状态（✅⚠️❌）
  - 异常项目高亮显示
- **单项目详情**（点击进入）:
  - 功能完成情况（P0/P1/P2 任务列表）
  - Agent 协作日志（谁在做什么，最新提交）
  - 质量指标（测试覆盖率、性能分数）
  - 风险预警（阻塞问题、延期风险）
- **自动刷新**: 数据实时更新，无需手动刷新

**成功标准**: 30 秒内了解所有项目的整体状况。
...
**故事**: Agent 遇到需要人工决策的问题（产品方向、技术选型），需要用户介入。

**用户期望**:
- **通知系统**:
  - 收到通知："ErgoBot 项目需要你的决策"
  - 点击跳转到具体问题
- **决策界面**:
  - 产品经理 Agent 提出的问题
  - 相关背景和选项
...
**成功标准**: 快速响应 Agent 的决策请求，不阻塞项目进展。

---

#### C4. Agent 协作可视化 — "看乐队排练"

**频率**: 每天 1-2 次（好奇时）

**故事**: 想看看 Agent 团队是如何协作的，有没有沟通问题。

**用户期望**:
- **协作日志**:
  - 时间线展示 Agent 之间的交互
  - 产品经理 Agent：生成了 PRD v1.0
  - 全栈 Agent：领取任务"实现登录功能"
  - QA Agent：发现测试覆盖率不足，通知全栈 Agent
- **协作关系图**（可选）:
  - 可视化 Agent 之间的协作关系
  - 识别协作瓶颈

**成功标准**: 能看到 Agent 的工作过程，建立信任感。
...
**故事**: Agent 团队完成了一个阶段的开发，用户需要审查代码并决定是否发布。

**用户期望**:
- **审查界面**:
  - 查看本次迭代的代码变更（Git diff）
  - 查看测试报告（通过率、覆盖率）
  - 查看性能报告（Lighthouse 分数）
- **审批流程**:
  - 批准：自动触发发布流程
  - 驳回：留下反馈，Agent 修改
- **发布确认**:
...
**注意**: 模块零（AI 项目管理）为 v2.x 战略方向，当前 v1.x 聚焦模块一和模块二。
...
| 功能 | 对应场景 | 用户价值 |
|------|----------|----------|
| 项目列表 Dashboard | C2 看总谱 | 多项目一览无余 |
| 项目创建 & Agent 团队组建 | C1 新建乐队 | 快速启动新项目 |
| 项目进展详情 | C2 单项目视图 | 实时掌握研发进展 |

**v2.1 - Agent 协作透明化**

| 功能 | 对应场景 | 用户价值 |
|------|----------|----------|
| Agent 协作日志 | C4 看排练 | 了解 Agent 工作过程 |
| 关键决策通知 | C3 指挥家介入 | 及时响应 Agent 需求 |

**v2.2 - 发布流程自动化**

| 功能 | 对应场景 | 用户价值 |
|------|----------|----------|
| 代码审查界面 | C5 总彩排 | 快速审查 Agent 代码 |
| 一键发布 | C5 发布 | 自动化部署流程 |
