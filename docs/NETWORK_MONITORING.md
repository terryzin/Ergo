# 网络状态监控优化

## 📊 优化概述

**v1.3.0+** 将网络状态监控分离为两个独立的指标，提供更精确的系统状态反馈。

### 之前（单一指标）
```
网络: 缓存  ← 混淆了连通性和数据新鲜度
```

### 优化后（双指标）
```
网络: 在线 (25ms)  ← 实时检测 API Bridge 连通性
数据: 缓存 (142s)  ← 显示数据新鲜度和缓存年龄
```

---

## 🎯 设计理念

### 职责分离
| 指标 | 含义 | 检测方式 | 更新频率 |
|------|------|----------|----------|
| **网络** | API Bridge 是否可达 | `/api/health` 快速检测 | 每 3 秒 |
| **数据** | 数据是实时还是缓存 | `_meta.cached` 标志 | 每 10 秒 |

### 用户价值
- ✅ **即时反馈** - 3秒内知道网络是否正常
- ✅ **精确状态** - 清楚区分连通性和数据新鲜度
- ✅ **延迟可见** - 显示网络响应时间（毫秒）
- ✅ **缓存透明** - 显示数据缓存了多久

---

## 🔧 技术实现

### 前端实现

**网络健康检查（新增）：**
```javascript
async function checkNetworkHealth() {
    try {
        const start = Date.now();
        const response = await fetch('/api/health', {
            method: 'GET',
            signal: AbortSignal.timeout(3000)  // 3秒超时
        });

        const latency = Date.now() - start;

        if (response.ok) {
            document.getElementById('networkStatus').textContent = '在线';
            document.getElementById('networkPill').innerHTML =
                `<span class="status-dot"></span><span>${latency}ms</span>`;
            document.getElementById('networkPill').className = 'status-pill ok';
            return true;
        }
    } catch (error) {
        document.getElementById('networkStatus').textContent = '离线';
        document.getElementById('networkPill').innerHTML = '<span>连接失败</span>';
        document.getElementById('networkPill').className = 'status-pill err';
        return false;
    }
}

// 每 3 秒检测一次
setInterval(checkNetworkHealth, 3000);
```

**数据状态显示（修改）：**
```javascript
async function loadData(forceRefresh = false) {
    const data = await fetchAllData(forceRefresh);
    const isCached = data._meta?.cached || false;
    const cacheAge = data._meta?.cacheAge;

    if (hasError) {
        document.getElementById('dataStatus').textContent = '异常';
        document.getElementById('dataPill').innerHTML = '<span>加载失败</span>';
        document.getElementById('dataPill').className = 'status-pill err';
    } else {
        const statusText = isCached ? '缓存' : '实时';
        const pillText = isCached && cacheAge ? `缓存 ${cacheAge}s` : statusText;

        document.getElementById('dataStatus').textContent = statusText;
        document.getElementById('dataPill').innerHTML =
            `<span class="status-dot"></span><span>${pillText}</span>`;
        document.getElementById('dataPill').className =
            isCached ? 'status-pill' : 'status-pill ok';
    }
}

// 每 10 秒刷新一次
setInterval(loadData, 10000);
```

### 后端实现

**API Bridge 新增端点：**
```javascript
// 认证中间件 - 豁免 health 端点
function authMiddleware(req, res, next) {
    if (req.path === '/health' || req.path === '/api/health') {
        return next();  // 无需认证
    }
    // ... 其他路由需要认证
}

/**
 * GET /api/health
 * API 健康检查（无需认证，用于前端快速检测）
 */
app.get('/api/health', (req, res) => {
    res.json({
        status: 'ok',
        timestamp: new Date().toISOString()
    });
});
```

---

## 📱 UI 展示

### KPI 卡片布局

```
┌─────────────────────────────────────────────┐
│  Gateway         Agents         Cron        │
│  运行中          3 个            5 个        │
│  ● 在线          ● 3 在线        ● 4 运行中  │
├─────────────────────────────────────────────┤
│  网络            数据                        │
│  在线            缓存                        │
│  ● 25ms          ● 缓存 142s                │
└─────────────────────────────────────────────┘
```

### 状态颜色

| 网络状态 | 显示 | 颜色 |
|----------|------|------|
| 在线 | 在线 (XXms) | 绿色 ● |
| 离线 | 连接失败 | 红色 ⚠️ |

| 数据状态 | 显示 | 颜色 |
|----------|------|------|
| 实时 | 实时 | 绿色 ● |
| 缓存 | 缓存 (XXs) | 蓝色 ● |
| 异常 | 加载失败 | 红色 ⚠️ |

---

## 🚀 性能与资源

### 网络开销

**之前（单一检测）：**
```
每 10 秒：1 次 /api/status 请求（需认证，~500ms）
```

**优化后（双重检测）：**
```
每 3 秒： 1 次 /api/health 请求（无需认证，~25ms）
每 10 秒：1 次 /api/status 请求（需认证，~500ms）
```

**分析：**
- 网络检测频率提高 3.3 倍（10s → 3s）
- 单次延迟降低 20 倍（500ms → 25ms）
- 带宽增加：每分钟增加 ~2KB（20 次 health 请求）
- **总体影响：** 微小（< 0.1 KB/s），用户体验显著提升

### 服务器负载

**API Bridge 负载：**
```
/api/health 端点：
- 无需认证检查
- 无需 CLI 调用
- 无需数据库查询
- 仅返回 JSON：~60 字节
```

**估算响应时间：**
- 本地访问：< 10ms
- 公网访问（cpolar）：20-50ms

---

## 🎯 使用场景

### 场景 1：网络故障诊断

**问题：** 页面显示"加载失败"

**排查：**
1. 查看"网络"指标：
   - ✅ **在线** → API Bridge 正常，问题在 Gateway
   - ❌ **离线** → API Bridge 故障或 cpolar 隧道断开

2. 查看"数据"指标：
   - **异常** → Gateway 不可达或 CLI 失败

### 场景 2：缓存验证

**问题：** 数据没有更新

**排查：**
1. 查看"数据"指标：
   - **缓存 (280s)** → 接近 5 分钟，即将自动刷新
   - **缓存 (30s)** → 刚刷新不久，数据应该是最新的

2. 手动刷新：
   - 点击"刷新"按钮
   - 观察"数据"变为"实时"

### 场景 3：性能监控

**目标：** 监控网络延迟

**方法：**
- 观察"网络"指标的延迟值
- **< 50ms** → 本地访问，性能优秀
- **50-200ms** → 公网访问，性能良好
- **> 200ms** → 网络缓慢，可能有问题

---

## 📊 监控指标说明

### 网络指标

| 显示 | 含义 | 操作建议 |
|------|------|----------|
| 在线 (10-50ms) | API Bridge 响应正常 | ✅ 无需操作 |
| 在线 (50-200ms) | 网络有延迟但可用 | ⚠️ 关注性能 |
| 在线 (>200ms) | 网络缓慢 | ⚠️ 检查网络或 cpolar |
| 离线 | API Bridge 不可达 | ❌ 检查服务和隧道 |

### 数据指标

| 显示 | 含义 | 操作建议 |
|------|------|----------|
| 实时 | 数据刚从 Gateway 查询 | ✅ 数据最新 |
| 缓存 (0-60s) | 1 分钟内的缓存 | ✅ 数据足够新 |
| 缓存 (60-240s) | 2-4 分钟的缓存 | ℹ️ 数据仍然有效 |
| 缓存 (240-300s) | 即将刷新 | ⏳ 等待自动刷新 |
| 异常 | 数据加载失败 | ❌ 检查 Gateway |

---

## 🧪 测试验证

### 本地测试

**1. 网络健康检查：**
```bash
# 测试 health 端点
curl http://localhost:8081/api/health
# 预期：{"status":"ok","timestamp":"..."}

# 测试响应时间
time curl -s http://localhost:8081/api/health > /dev/null
# 预期：< 0.05s
```

**2. 数据状态检查：**
```bash
# 查询数据状态
curl -H "X-Ergo-Key: ergo-default-secret-key-2026" \
     http://localhost:8081/api/status | jq '._meta'
# 预期：{"cached": true, "cacheAge": 120, ...}
```

### 公网测试

**在浏览器中访问：**
```
https://terryzin.cpolar.cn
```

**验证要点：**
1. 打开开发者工具（F12）→ Network 标签
2. 观察 `/api/health` 请求：
   - 频率：每 3 秒一次
   - 响应时间：< 100ms
   - 状态码：200 OK

3. 观察前端显示：
   - "网络"显示"在线 (XXms)"
   - "数据"显示"缓存 (XXs)"或"实时"

---

## 🔄 回滚方案

如果新方案有问题，可以回滚：

```bash
# 回滚到之前的版本
git revert d0437b8

# 或者检出旧版本
git checkout b308459
```

**回滚影响：**
- ❌ 失去网络实时监控
- ❌ 恢复到单一"缓存"显示
- ✅ 基础功能不受影响

---

## 📝 未来优化

### 可能的改进

1. **自适应检测频率**
   ```javascript
   // 网络正常：每 5 秒检测
   // 网络异常：每 1 秒重试
   const interval = networkHealthy ? 5000 : 1000;
   ```

2. **延迟告警**
   ```javascript
   if (latency > 200) {
       showToast('网络延迟较高', 'warn');
   }
   ```

3. **历史趋势图**
   - 记录最近 1 小时的延迟数据
   - 绘制折线图显示趋势

4. **智能重连**
   - 网络恢复后自动刷新数据
   - 避免用户手动操作

---

**更新日期：** 2026-02-20
**版本：** v1.3.0+
**作者：** Ergo Team
**状态：** ✅ 已实施并推送
